...
function GR:RemoveFilter(filterName)
...
function GR:TestCharacters(word)
	local V = GetMaxNumberConsequtiveVowels(word);
	local C = GetMaxNumberConsequtiveConsonants(word);
	
	GR:print("Vowels: "..V.." Consonants: "..C);
end

local function debugPlayer(player)
	local text = "|cff"..GR:GetClassColor(player.class)..player.name.."|r ("..player.level..") ("..player.race..")";
	return text
end
...
local function match(filter, player)
	...
	if (type(filter.race) == "table" and GR:CountTable(filter.race) ~= 0) then
		if (not filter.race[player.race]) then
			return false;
		end
	end
	
	if (type(filter.class) == "table" and GR:CountTable(filter.class) ~= 0) then
	...
	GR:debug("Filter: "..filter.nameOfFilter.." "..debugPlayer(player));
	return true;
...
function GR:FilterPlayer(player)
	...
	if (type(filters) ~= "table" or type(player) ~= "table") then
		...
		GR:debug("Error: Filter or player is not a table!");
		return false;
	end
