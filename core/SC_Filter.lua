...
local function AddFilter(filter)
	tinsert(SC_DATA[SC_DATA_INDEX].settings.filters, filter);
end

function SC:CreateFilter(nameOfFilter,class, name, minLvl, maxLvl, race, maxVowels, maxConsonants, active)
...
function SC:RemoveFilter(filterName)
...
function SC:TestCharacters(word)
	local V = GetMaxNumberConsequtiveVowels(word);
	local C = GetMaxNumberConsequtiveConsonants(word);
	
	SC:print("Vowels: "..V.." Consonants: "..C);
end

local function debugPlayer(player)
	local text = "|cff"..SC:GetClassColor(player.class)..player.name.."|r ("..player.level..") ("..player.race..")";
	return text
end
...
local function match(filter, player)
	...
	if (type(filter.race) == "table" and SC:CountTable(filter.race) ~= 0) then
		if (not filter.race[player.race]) then
			return false;
		end
	end
	
	if (type(filter.class) == "table" and SC:CountTable(filter.class) ~= 0) then
	...
	SC:debug("Filter: "..filter.nameOfFilter.." "..debugPlayer(player));
	return true;
	
end

local function GetActiveFilters()
	local filters, activeFilters = SC_DATA[SC_DATA_INDEX].settings.filters, {};
...
function SC:FilterPlayer(player)
	local filters = GetActiveFilters();--SC_DATA[SC_DATA_INDEX].settings.filters;
	
	if (type(filters) ~= "table" or type(player) ~= "table") then
		--if invalid parameter, return as filtered
		SC:debug("Error: Filter or player is not a table!");
		return false;
	end
